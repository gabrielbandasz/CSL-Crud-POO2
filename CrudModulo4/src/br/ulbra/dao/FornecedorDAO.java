...
public class FornecedorDAO {
    ...
    public Fornecedor obterPorId(int id) throws SQLException {
        String sql = "SELECT idfor, nomerazao, nomefantasia, telefone, email FROM tbfornecedor WHERE id = ?";
        try (Connection con = getConnection();
    ...
    public void atualizar(Fornecedor f) throws SQLException {
        String sql = "UPDATE tbfornecedor SET nomerazao = ?, nomefantasia = ?, telefone = ?, email = ? WHERE id = ?";
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
